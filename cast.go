...
func ToDuration(i interface{}) (d time.Duration, err error) {
	...
	case int, int64, int32, int16, int8, uint, uint64, uint32, uint16, uint8:
		d = time.Duration(ToInt64IgnoreError(s))
		return
	case float32, float64:
		d = time.Duration(ToFloat64IgnoreError(s))
		return
	case string:
...
func ToStringMapString(i interface{}) (map[string]string, error) {
	...
	case map[string]interface{}:
		for k, val := range v {
			m[ToStringIgnoreError(k)] = ToStringIgnoreError(val)
		}
		return m, nil
...
func ToStringMapStringSlice(i interface{}) (map[string][]string, error) {
	...
	case map[string][]interface{}:
		for k, val := range v {
			m[ToStringIgnoreError(k)] = ToStringSliceIgnoreError(val)
		}
		return m, nil
	case map[string]string:
		for k, val := range v {
			m[ToStringIgnoreError(k)] = []string{val}
		}
	case map[string]interface{}:
		for k, val := range v {
			switch vt := val.(type) {
			case []interface{}:
				m[ToStringIgnoreError(k)] = ToStringSliceIgnoreError(vt)
			case []string:
				m[ToStringIgnoreError(k)] = vt
			default:
				m[ToStringIgnoreError(k)] = []string{ToStringIgnoreError(val)}
			}
		}
		return m, nil
	case map[interface{}][]string:
...
func ToStringMapBool(i interface{}) (map[string]bool, error) {
	...
	case map[interface{}]interface{}:
		for k, val := range v {
			m[ToStringIgnoreError(k)] = ToBoolIgnoreError(val)
		}
		return m, nil
	case map[string]interface{}:
		for k, val := range v {
...
func ToStringMapInt(i interface{}) (map[string]int, error) {
	...
	case map[interface{}]interface{}:
		for k, val := range v {
			m[ToStringIgnoreError(k)] = ToIntIgnoreError(val)
		}
		return m, nil
	case map[string]interface{}:
		for k, val := range v {
			m[k] = ToIntIgnoreError(val)
		}
		return m, nil
	case map[string]int:
		return v, nil
	...
	for _, keyVal := range v.MapKeys() {
		val, err := ToInt(v.MapIndex(keyVal).Interface())
		if err != nil {
			return m, fmt.Errorf("unable to cast %#v of type %T to map[string]int", i, i)
		}
...
func ToStringMapInt64(i interface{}) (map[string]int64, error) {
	...
	case map[interface{}]interface{}:
		for k, val := range v {
			m[ToStringIgnoreError(k)] = ToInt64IgnoreError(val)
		}
		return m, nil
	case map[string]interface{}:
		for k, val := range v {
			m[k] = ToInt64IgnoreError(val)
		}
		return m, nil
	...
	v := reflect.ValueOf(i)
	for _, keyVal := range v.MapKeys() {
		val, err := ToInt64(v.MapIndex(keyVal).Interface())
		if err != nil {
			return m, fmt.Errorf("unable to cast %#v of type %T to map[string]int64", i, i)
		}
		mVal.SetMapIndex(keyVal, reflect.ValueOf(val))
...
func ToStringSlice(i interface{}) ([]string, error) {
	...
	case []interface{}:
		for _, u := range v {
			a = append(a, ToStringIgnoreError(u))
		}
		return a, nil
	case []string:
		return v, nil
